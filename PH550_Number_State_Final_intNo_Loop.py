...
L = 9 # number of modes that are being detected (L/K is the quantum efficiency)
N = 2 # Number of photons emitted per coherence time (assumed fluctuation free)
intNo = 1 # integration time / coherence time
intNoMax = 20
intSumsArraySDs = np.zeros(intNoMax)

while intNo < (intNoMax+1):
    ...
    reps = 10
    chosenStates = df.iloc[:, : (int((L/K)*reps))]
    MsumOfSCS = np.zeros(len(chosenStates)*reps)
    i=0
    while i<reps:
    ...
    i = intNo-1
    intSumsArraySDs[i] = intSumsArraySD
    intNo = intNo+1
