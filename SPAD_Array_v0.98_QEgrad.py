...
def LoopPlots():
    ...
    for col in df.columns:
        ...
        try:
            sns.distplot(r, ax=axs[i][j], hist=False, fit=None, kde=False, color="r")
        ...
        axs[i][j].set_title("Sensor "+str(k+1)+", QE = "+str(round(QE*(1-reduction[k]),2)))
...
ax1.set_xlabel('Pixel Number', fontsize=16)
ax1.set_xticks(sensor_names)
ax1.set_ylabel('FWHM of Pixel', fontsize=16, color='red')
ax1.plot(sensor_names, FWHM_pixel, color='red',marker='o', ms=10, linestyle='None')
ax1.tick_params(axis='y', labelcolor='red')
ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
ax2.set_ylabel('FWHM/Mean for Pixel', fontsize=16, color='lightseagreen')  # we already handled the x-label with ax1
...
fig.suptitle("FWHM and FWHM/Mean per Pixel", fontsize=18)
