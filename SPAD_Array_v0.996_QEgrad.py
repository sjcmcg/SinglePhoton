...
total_system_events = (detector_events == 1).astype(int).sum(axis=0)
...
if user_sensors > hists_per_row:
    ...
    fig.text(0.04, 0.5, 'Photon Detection Probability', va='center', rotation='vertical', fontsize=28)
    for col in df.columns:
        r = poisson.rvs(np.multiply(hist_n,df[col]).sum(), size=N)
...
ax2.plot(sensor_names, np.log10(FWHM_o_mean), color='lightseagreen', marker='o', ms=10, linestyle='None')
...
plt.scatter(sensor_names, fwhmA_fwhmP, alpha=0.7, color='lightseagreen', s=20)
