...
N = 300000 # number of samples taken
rLineValuesPixel = []
...
def LoopPlots():
    ...
    for col in df.columns:
        ...
        rLineValuesPixel.append(sns.distplot(r, hist=False, fit=norm, fit_kws={"color":"black"}, kde=False, color="r", label='Poisson Distribution for Same Mean').get_lines()[0].get_data())
...
while True:
    try:
      ...
      break
        ...
        continue
# User input determines number of sensors using the linspace below
...
if user_sensors > hists_per_row:
    ...
    fig, axs = plt.subplots(1,user_sensors)
    fig.text(0.5, 0.04, 'Number of Photons Detected', ha='center', fontsize=28)
    fig.text(0.04, 0.5, 'Probability of D Photons being Detected', va='center', rotation='vertical', fontsize=28)
    for col in df.columns:
        r = poisson.rvs(np.multiply(hist_n,df[col]).sum(), size=N)
        xint = np.arange(0,math.ceil(max(r)+1))
...
h0.tick_params(axis='both', which='major', labelsize=28)
# Shade FWHM area using calculated values
h0.axvspan(half_max_pos1, half_max_pos2, facecolor='g', alpha=0.5)
# h0.set_xlabel("Number of detected photons (D)", fontsize=20)
h0.set_ylabel("Probability of detecting D photons", fontsize=25)
...
h1.set_xticks(xint)
h1.tick_params(axis='both', which='major', labelsize=28)
h1.set_xlabel("Number of detected photons (D)", fontsize=25)
h1.set_ylabel("Residuals", fontsize=25)
h1.text(np.max(xint), np.max(totalCountResiduals)*0.5, 'RSS = ' + str(sum_squares), ha='center', fontsize=28)
f.suptitle("Overall probability for full array with QE = " +  str(QE), fontsize=30)
f.show()
# input('press <ENTER> to continue')

# # plot FWHM vs. Pixel Number 
# plt.figure()
# plt.title("FWHM per sensor with QE = " +  str(QE), fontsize=18)
# plt.scatter(sensor_names, FWHM_pixel, alpha=0.7)
# plt.xlabel("Pixel Number", fontsize=18)
# plt.ylabel("FWHM of Pixel Detection", fontsize=16)
# plt.ylim(bottom=0)
# plt.ylim(top=1.1*np.max(FWHM_pixel))
# plt.show()

# # plot FWHM/mean vs. Pixel Number 
# plt.figure()
# plt.title("FWHM per sensor with QE = " +  str(QE), fontsize=18)
# plt.scatter(sensor_names, FWHM_o_mean, alpha=0.7, color='lightseagreen')
# plt.xlabel("Pixel Number", fontsize=18)
# plt.ylabel("FWHM/Mean of Pixel", fontsize=16)
# plt.ylim(bottom=0)
...
plt.figure()
plt.title("FWHM of Senor divided by FWHM of Poisson", fontsize=30)
plt.scatter(sensor_names, fwhmA_fwhmP, alpha=0.7, color='lightseagreen')
plt.xlabel("Pixel Number", fontsize=28)
plt.ylabel("FWHM/Poisson FWHM", fontsize=28)
plt.ylim(bottom=0)
plt.ylim(top=1.1*np.max(FWHM_o_mean))
plt.xticks(fontsize=28)
plt.xticks(sensor_names)
plt.show()